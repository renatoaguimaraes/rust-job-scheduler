...
use worker::{
    ListRequest, ListResponse, QueryRequest, QueryResponse, StartRequest, StartResponse,
...
impl WorkerService for WorkerServiceServerImpl {
    ...
    async fn stream(
        ...
        let (mut tx_log, mut rx_log) = mpsc::channel(1);
    ...
    async fn list(
        ...
        info!("{:?}", request);
        Ok(Response::new(ListResponse { processes: vec![] }))
    }
}
